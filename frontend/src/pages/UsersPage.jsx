...
const UsersPage = () => {
    ...
    useEffect(() => { if (currentUser) fetchUsers(); }, [selectedRole, page, pageSize, currentUser, showArchived]);
    ...
    const exitSelectionMode = () => { setSelectionMode(false); setSelectedUsers([]); };
    ...
    const renderDeleteModalText = () => {
        ...
        if (showArchived) {
            return {
                ...
                desc: isBulk ? 'This will permanently delete selected users. This action cannot be undone.'
                    : `This will permanently delete "${confirmUsers[0]?.name}". This cannot be undone.`
            };
        }
        return {
            title: isBulk ? `Archive ${confirmUsers.length} users?` : 'Archive user?',
            desc: isBulk ? 'Selected users will be moved to archive. You can restore them later.'
                : `"${confirmUsers[0]?.name}" will be moved to archive.`
        };
    };
        ...
        <DashboardLayout>
            ...
            {message.text && (
                <div className={`fixed top-6 right-6 z-[100] px-5 py-3.5 rounded-lg shadow-lg flex items-center gap-3 animate-fadeIn backdrop-blur-sm ${message.type === 'success' ? 'bg-emerald-500/90 text-white' : 'bg-red-500/90 text-white'}`}>
                    ...
                    <button onClick={() => setMessage({ type: '', text: '' })} className="ml-2 p-1.5 hover:bg-white/20 rounded-md transition-colors">
                        <FaTimes size={10} />
                    </button>
                </div>
            )}
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        ...
                        <p className="text-gray-500 text-sm mt-0.5">
                            {showArchived ? 'Manage archived users - restore or permanently delete'
                                                ...
                                                <div className="flex items-center gap-2 relative">
                                                    ...
                                                    {showSortMenu && (
                                                        <div ref={sortMenuRef} className="absolute top-full left-0 mt-1 w-32 bg-white rounded-lg shadow-xl border border-gray-100 py-1 z-50 animate-fadeIn">
                                                            ...
                                                            {SORT_OPTIONS.filter(o => !o.requireMultiRole || allowedRoles.length > 1).map(renderSortButton)}
                                                            {sortBy !== 'default' && (
                                                                <>
            ...
            {/* Modals */}
            <AddUserModal isOpen={!!activeModal} onClose={() => setActiveModal(null)} roleToAdd={activeModal} onSuccess={handleUserCreated} />
                ...
                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-xl shadow-2xl max-w-md w-full overflow-hidden animate-fadeIn">
                        <div className="p-5 border-b border-gray-100 flex justify-between items-center">
                            ...
                            <button onClick={() => setEditModal({ open: false, user: null })} className="p-1.5 hover:bg-gray-100 rounded-md transition-colors text-gray-400 hover:text-gray-600">
                                <FaTimes size={12} />
                            </button>
                        </div>
                        <div className="p-5">
