...
const Calendar = () => {
  ...
  const [formData, setFormData] = useState({ title: '', startDate: '', endDate: '', type: 'event', description: '', allDay: true });
  ...
  const holidayCount = useMemo(() => {
    ...
    events.forEach(event => {
      ...
      const current = new Date(start);
      while (current <= end) {
        if (current >= monthStart && current <= monthEnd) uniqueDays.add(formatDate(current));
        current.setDate(current.getDate() + 1);
      }
    });
  ...
  const handleDayClick = (day) => {
    ...
    if (existing?.length > 0) {
      ...
      setFormData({ _id: event._id, title: event.title, startDate: formatDate(new Date(event.start)), endDate: formatDate(new Date(event.end)), type: event.type || 'event', description: event.description || '', allDay: event.allDay !== false });
    } else {
      setFormData({ title: '', startDate: dateStr, endDate: dateStr, type: 'event', description: '', allDay: true });
    }
    setShowModal(true);
  };
  ...
  const handleSaveEvent = async () => {
    ...
    try {
      setSaving(true);
      const payload = { title: formData.title.trim(), start: new Date(formData.startDate).toISOString(), end: new Date(formData.endDate).toISOString(), allDay: formData.allDay, type: formData.type, description: formData.description };
              ...
              <div className="space-y-2 text-xs text-gray-500">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`px-2 py-0.5 rounded text-[10px] font-medium ${getTypeColors(hoveredEvent.type).light}`}>
                    {TYPE_CONFIG[hoveredEvent.type]?.label || 'Event'}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                ...
                {hoveredEvent.description && (
                  <div className="pt-1 border-t border-gray-100">
                    <p className={expandedDescription ? '' : 'line-clamp-2'}>{hoveredEvent.description}</p>
                    {hoveredEvent.description.length > 80 && (
                      <button onClick={(e) => { e.stopPropagation(); setExpandedDescription(!expandedDescription); }}
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <div>
            <h1 className="text-lg font-bold text-gray-800 flex items-center gap-2">
              <FaCalendarAlt className="text-indigo-600" /> Academic Calendar
            </h1>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden">
            ...
            <div className="p-5 space-y-4">
              {renderFormField('Event Title',
                <input type="text" value={formData.title} onChange={(e) => updateFormField('title', e.target.value)} placeholder="e.g., Annual Sports Day" autoFocus
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                {renderFormField('End Date',
                  <input type="date" value={formData.endDate} min={formData.startDate} onChange={(e) => updateFormField('endDate', e.target.value)}
                    className="w-full px-3 py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-sm" />
                )}
              </div>
              {renderFormField('Event Type',
                <select value={formData.type} onChange={(e) => updateFormField('type', e.target.value)}
              ...
              {renderFormField('Description',
                <textarea value={formData.description} onChange={(e) => updateFormField('description', e.target.value)} placeholder="Add event details..." rows={3}
